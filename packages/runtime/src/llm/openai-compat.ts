...
export class OpenAICompatProvider implements LLMProvider {
	...
	async chat(request: LLMRequest): Promise<LLMResponse> {
		const params = this.toOpenAIParams(request);
		...
		for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
			try {
				...
				if (isRateLimitError(err)) {
					...
					throw new RateLimitError("openai-compat", retryAfter);
				}
				throw new LLMError(
...
function toOpenAIMessage(msg: LLMMessage): OpenAI.ChatCompletionMessageParam {
	...
	if (msg.role === "user") {
		...
		if (toolResults.length > 0) {
			...
			// Multiple tool results need multiple messages, but we handle
			// one at a time since the ReAct loop sends one result per message
